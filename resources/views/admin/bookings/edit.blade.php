...
<div class="space-y-6">
    <div>
        ...
        <h2 class="page-title">Chỉnh sửa đặt lịch</h2>
        <p class="page-subtitle">Cập nhật thông tin lịch tập</p>
    </div>

    <form id="booking-form" action="{{ route('admin.bookings.update', $booking->id) }}" method="POST" class="glass-card p-6 space-y-8 animate-fade-in-up">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div>
                ...
                <input name="booking_date" type="date" value="{{ old('booking_date', $booking->booking_date) }}" class="admin-input @error('booking_date') input-error @enderror" />
                @error('booking_date') <p class="mt-1.5 text-xs text-red-400">{{ $message }}</p> @enderror
            </div>
            <div>
                <label class="admin-label">Bắt đầu</label>
                <input name="start_time" type="time" value="{{ old('start_time', $booking->start_time) }}" class="admin-input @error('start_time') input-error @enderror" />
                @error('start_time') <p class="mt-1.5 text-xs text-red-400">{{ $message }}</p> @enderror
            </div>
            <div>
                <label class="admin-label">Kết thúc</label>
                <input name="end_time" type="time" value="{{ old('end_time', $booking->end_time) }}" class="admin-input @error('end_time') input-error @enderror" />
