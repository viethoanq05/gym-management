...
@section('content')
<div class="space-y-6">
    <h2 class="text-2xl font-semibold">Tạo đặt lịch</h2>

    <form action="{{ route('admin.bookings.store') }}" method="POST" class="space-y-6 bg-white p-6 rounded-xl shadow">
        @csrf
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input name="booking_date" type="date" value="{{ old('booking_date') }}" class="mt-1 w-full border rounded px-3 py-2" />
                @error('booking_date')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            <div>
                ...
                <input name="start_time" type="time" value="{{ old('start_time') }}" class="mt-1 w-full border rounded px-3 py-2" />
                @error('start_time')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            <div>
                ...
                <input name="end_time" type="time" value="{{ old('end_time') }}" class="mt-1 w-full border rounded px-3 py-2" />
                @error('end_time')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
