        ...
        <div>
            ...
            <div class="mt-3 grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input name="specialization" value="{{ old('specialization', optional($user->trainer)->specialization) }}" class="mt-1 w-full border rounded px-3 py-2 @error('specialization') border-red-500 bg-rose-50 @enderror" />
                    @error('specialization') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="block text-sm">Kinh nghiệm (năm)</label>
                    <input name="experience_years" value="{{ old('experience_years', optional($user->trainer)->experience_years) }}" class="mt-1 w-full border rounded px-3 py-2 @error('experience_years') border-red-500 bg-rose-50 @enderror" />
                    @error('experience_years') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                </div>
                <div class="col-span-2">
                    <label class="block text-sm">Mô tả</label>
                    <textarea name="description" class="mt-1 w-full border rounded px-3 py-2 @error('description') border-red-500 bg-rose-50 @enderror">{{ old('description', optional($user->trainer)->description) }}</textarea>
