    ...
    <form action="{{ route('admin.packages.store') }}" method="POST" class="space-y-6 bg-white p-6 rounded-xl shadow">
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input name="price" value="{{ old('price') }}" class="mt-1 w-full border rounded px-3 py-2 @error('price') border-red-500 bg-rose-50 @enderror" />
                @error('price')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            <div>
                ...
                <input name="duration_days" value="{{ old('duration_days') }}" class="mt-1 w-full border rounded px-3 py-2 @error('duration_days') border-red-500 bg-rose-50 @enderror" />
                @error('duration_days')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="col-span-2">
                ...
                <textarea name="description" class="mt-1 w-full border rounded px-3 py-2 @error('description') border-red-500 bg-rose-50 @enderror" rows="4">{{ old('description') }}</textarea>
                @error('description')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
