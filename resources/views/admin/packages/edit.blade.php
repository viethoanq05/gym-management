    ...
    <form id="package-form" action="{{ route('admin.packages.update', $package->id) }}" method="POST" class="space-y-6 bg-white p-6 rounded-xl shadow">
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input name="name" value="{{ old('name', $package->name) }}" class="mt-1 w-full border rounded px-3 py-2" />
                @error('name')
            ...
            <div>
                ...
                <input name="price" value="{{ old('price', $package->price) }}" class="mt-1 w-full border rounded px-3 py-2" />
                @error('price')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            <div>
                ...
                <input name="duration_days" value="{{ old('duration_days', $package->duration_days) }}" class="mt-1 w-full border rounded px-3 py-2" />
                @error('duration_days')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="col-span-2">
                ...
                <textarea name="description" class="mt-1 w-full border rounded px-3 py-2" rows="4">{{ old('description', $package->description) }}</textarea>
                @error('description')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
