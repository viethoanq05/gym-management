...
<body class="min-h-screen bg-slate-50">
    ...
    <main class="max-w-5xl mx-auto mt-10 p-4">
        <div class="flex justify-between items-end mb-6">
            <div>
                <h1 class="text-2xl font-extrabold text-slate-800">Quản lý Đăng ký Gói</h1>
                <p class="text-sm text-slate-500 mt-1">Duyệt, bảo lưu, hoặc hủy các gói tập của hội viên.</p>
            </div>
            <a href="{{ route('staff.memberships.assign') }}" class="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold rounded-lg shadow-sm transition">
                + Chọn Gói Cho Hội Viên
            </a>
        </div>
                    ...
                    <tbody class="divide-y divide-slate-100 border-t border-slate-100">
                        @forelse($memberships as $ms)
                            <tr class="hover:bg-slate-50/70 transition">
                                <td class="px-6 py-4">
                                    <p class="font-semibold text-slate-800">{{ $ms->user_name }}</p>
                                    <p class="text-xs text-slate-400 mt-0.5">{{ $ms->user_phone }}</p>
                                </td>
                                <td class="px-6 py-4 font-medium text-blue-600">
                                    {{ $ms->package_name }}
                                </td>
                                <td class="px-6 py-4 font-bold text-slate-700">
                                    {{ number_format($ms->package_price, 0, ',', '.') }}đ
                                </td>
                                <td class="px-6 py-4 text-xs text-slate-500">
                                    @if(isset($ms->start_date) && isset($ms->end_date))
                                        {{ \Carbon\Carbon::parse($ms->start_date)->format('d/m/Y') }} ➔ {{ \Carbon\Carbon::parse($ms->end_date)->format('d/m/Y') }}
                                    @else
                                        <span class="text-slate-400 italic">Chưa sắp lịch</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    @if($ms->status == 1)
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end gap-1.5">
                                        @if($ms->status == 0 || $ms->status == 'pending')
